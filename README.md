...
This project implements a Docker container for [fre:ac](https://www.freac.org).
...
`freac.domain.tld` to the container at `127.0.0.1:5800`.
...
`server.domain.tld/freac` to the container at `127.0.0.1:5800`.
...
is `/dev/sr1` and so on. To allow fre:ac to access the first drive,
