...
server {
	...
	location /freac/ {
		...
		location ~ ^/freac/(websockify(-.*)?) {
                        proxy_pass http://docker-freac/$1;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of fre:ac integrated into Docker image. |
...
Any desktop notification generated by fre:ac is forwarded to the
